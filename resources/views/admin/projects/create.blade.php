...
<section>
    ...
    <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="btn-group" role="group" aria-label="Basic checkbox toggle button group"> select tag:
            @foreach($tags as $tag)
                <input type="checkbox" class="btn-check " id="{{$tag->id}}" name="tags[]" value="{{$tag->id}}">
                <label class="btn btn-outline-primary mb-3" for="{{$tag->id}}">{{$tag->name}}</label>
            @endforeach
